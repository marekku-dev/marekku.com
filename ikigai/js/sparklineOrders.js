...
let spark = d3.select("#sparklineOrders")
  .append("svg")
...
d3.csv("/ikigai/data/orders.csv",
  ...
  // Приводим даты к универсальному формату
  function(d){
    return { date : d3.timeParse("%Y-%m-%d")(d.Date), value : d.OM }
  },
  ...
  function(data) {
    ...
    // Шкала абсцисс берёт все даты в массиве
    let x = d3.scaleTime()
      .domain(d3.extent(data, function(d) { return d.date; }))
      .range([ 0, width ]);

    // Шкала ординат с максимумом в 100 заказов
    ...
    // Число над кружочком при наведении
    let focusText = spark
    .append('g')
    .append('text')
      .style("opacity", 0)
    ...
    spark.append('text')
      ...
      .text(normalDate(data[14].date) + ' Orders')
